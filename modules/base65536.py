...
##############################  END HEADER  ##########################################
#
#[o_o]< not created by Artifact Dev, shipped with Torta as Parkayun's Base65536
#
#
global IMPORTED_BASE65536
IMPORTED_BASE65536 = 1
import os
if(os.path.exists('../data/verbose')): 
    print("...imported:['./modules/base65536.py']...")
...
def encode(value):
    ...
    if(type(value)==type('base')): value = value.encode()
    if(type(value)==type(65536)): value = str(value).encode()
    stream = io.StringIO()
    length = len(value)
